...
describe Actions::CreateServiceInstance do
  ...
  context "bosh uses a pool of InfrastructureNetworks" do
    ...
    it "generates deployment manifest and deploys it" do
      ...
      expect(bosh_director_client).to receive(:allocate_infrastructure_network).and_return(infrastructure_network)
      ...
      expect(service_instance).to receive(:infrastructure_network).and_return(nil)
      expect(service_instance).to receive(:infrastructure_network=).with(infrastructure_network)
      expect(service_instance).to receive(:save)
  ...
  context "bosh does not use InfrastructureNetworks" do
    let(:infrastructure_network){ nil }
    ...
    it "generates deployment manifest and deploys it" do
      ...
      expect(class_double('ServiceInstance').as_stubbed_const).to receive(:find_by_id).with(service_instance_id).and_return(service_instance)
      expect(service_instance).to receive(:infrastructure_network).exactly(2).times.and_return(nil)
