...
describe Actions::UpdateServiceInstanceState do
  let(:service_id)          { 'service-id' }
  let(:service_instance_id) { 'instance-id' }
  let(:service_instance)    { instance_double('ServiceInstance') }

  subject { Actions::UpdateServiceInstanceState.new(service_id: service_id, service_instance_id: service_instance_id) }
  ...
  context "ServiceInstance was deploying" do
    ...
    it "does checks latest BOSH task status, and is now running" do
      expect(director_client).to receive(:task_state).with(task_id).and_return(:done)
    ...
    it "does checks latest BOSH task status, and is still deploying" do
      expect(director_client).to receive(:task_state).with(task_id).and_return(:running)
    ...
    it "does checks latest BOSH task status, and deployment failed" do
      expect(director_client).to receive(:task_state).with(task_id).and_return(:failed)
